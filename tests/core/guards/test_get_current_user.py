...
from app.core.guards import get_current_user, JWT_SECRET, ALGORITHM

@pytest.mark.unit
def test_get_current_user_success(mock_db, fake_active_user):
    token = "fake_token"
    with patch("app.core.guards.jwt.decode", 
               return_value={"sub": fake_active_user.id}) as mock_decode, \
         patch("app.users.crud.get_user_by_id", 
               return_value=fake_active_user) as mock_get_user_by_id:

        result = get_current_user(token=token, db=mock_db)

        mock_decode.assert_called_once_with(token, JWT_SECRET, algorithms=[ALGORITHM])
        mock_get_user_by_id.assert_called_once_with(mock_db, user_id=fake_active_user.id)
...
def test_get_current_user_user_not_found(mock_db):
    token = "fake_token"
    with patch("app.core.guards.jwt.decode", return_value={"sub": 1}) as mock_decode, \
         patch("app.users.crud.get_user_by_id", return_value=None) as mock_get_user_by_id:
