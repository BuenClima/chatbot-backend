...
@pytest.mark.unit
def test_sign_up_should_return_200_and_access_token(
    override_get_db, override_get_current_user, mock_db
):
    ...
    with patch(
        "app.auth.service.sign_up_user", return_value=mock_response
    ) as mock_sign_up_user:
...
@pytest.mark.unit
def test_sign_up_should_raise_exception(
    override_get_db, override_get_current_user, mock_db
):
    user_data = {"email": "test@example.com", "hashed_password": "hashed_password"}
    ...
    with patch(
        "app.auth.service.sign_up_user",
        side_effect=HTTPException(status_code=400, detail="Error creating user"),
    ) as mock_sign_up_user:
        response = client.post("/auth/signup", json=user_data)
