...
def test_sign_in_user_should_return_200_and_access_token(mock_db):
    form_data = authschemas.AuthSignIn(email="test@example.com", hashed_password="hashed_password")
    db_user = userschemas.User(id=1, email="test@example.com", is_active=True,hashed_password="hashed_password")
    ...
    with patch("app.users.crud.get_user_by_email", return_value=db_user) as mock_get_user_by_email, \
         ...
         patch("app.core.security.create_tokens", return_value=tokens) as mock_create_tokens:
